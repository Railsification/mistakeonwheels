...
class HangmanCog(commands.Cog):
    GAME_META = {
        ...
        "kind": "solo",
        "result_word": "solve",
        "description": "Community Hangman with WoS words, custom words and shared guesses",
        "emoji": "❄️",
    ...
    HELP_META = {
        "title": "WoS Hangman",
        "summary": "A persistent community Hangman game with button guesses and WoS vault artwork.",
        "goal": "Solve the hidden word or phrase before the group reaches 7 misses.",
        "how_to_play": (
            "Start a random WoS puzzle with `/hangman`, choose Hangman from `/games`, "
            "or use `/hangman_custom` to enter a private custom answer. Anyone in the "
            "channel can press **Guess Letter** or **Guess Word**. Correct letters are "
            "revealed for everyone and the first player to complete the answer gets the solve."
        ),
        "rules": (
            "The game allows **7 misses**. A wrong letter costs 1 miss and a wrong full-word "
            "or phrase guess also costs 1 miss. Repeating an already-tried guess does not "
            "cost another miss. The game ends when the answer is solved, 7 misses are reached, "
            "or someone uses `/hangman_end`. Active games survive normal Railway restarts."
        ),
        "details": (
            "Use `/hangman` for a random WoS word, `/hangman_custom` for a private manual "
            "answer, or choose Hangman from `/games`. Anyone can guess and anyone can use "
            "`/hangman_end`."
        ),
    }
