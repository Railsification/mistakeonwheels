    ...
    def _build_embed(
        ...
        *,
        ...
        if status == "active":
            ...
            description = "Game finished. Both fleets are revealed below."
        else:
        ...
        if status in {"finished", "resigned"}:
            embed.add_field(
                name=f"🚢 {self._slot_name(game, 1)} — Final Fleet",
                value=f"```text\n{render_own_board(fleet1, shots2)}\n```",
                inline=False,
            )
            embed.add_field(
                name=f"🚢 {self._slot_name(game, 2)} — Final Fleet",
                value=f"```text\n{render_own_board(fleet2, shots1)}\n```",
                inline=False,
            )
        else:
            embed.add_field(
        ...
        if status == "active":
            ...
            embed.set_footer(
                text=(
                    ...
                    "My Fleet privately shows S = your ship."
                )
            )
            ...
            footer = "Final fleets: S = surviving ship • X = hit • o = miss • . = water"
            if bool(game.get("computer")):
                footer += " • Computer games are practice and do not affect the leaderboard."
