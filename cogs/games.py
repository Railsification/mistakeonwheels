...
# Existing public constant kept unchanged for compatibility.
TWO_PLAYER_GAMES = {"connect4", "tictactoe"}
COMMUNITY_GAMES = {"hangman", "rebus"}
...
class OpponentSelect(discord.ui.UserSelect):
    def __init__(self):
        super().__init__(
            placeholder="Pick an opponent (not needed for Hangman or Rebus)...",
            min_values=1,
            max_values=1,
            row=1,
        )
...
class GamesView(discord.ui.View):
    def __init__(self, bot: commands.Bot, author_id: int):
        ...
        # Started Hangman and Rebus games themselves have no timeout.
    ...
    def render_content(self) -> str:
        ...
        if self.selected_game in COMMUNITY_GAMES:
...
class StartButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if view.selected_game in COMMUNITY_GAMES:
            view.disable_all()
            await view.refresh(interaction)

            cog_name = "HangmanCog" if view.selected_game == "hangman" else "RebusCog"
            display_name = "Hangman" if view.selected_game == "hangman" else "Rebus"
            game_cog = interaction.client.get_cog(cog_name)

            if game_cog is None:
                await interaction.followup.send(
                    f"❌ {display_name} cog isn’t loaded.",
                    ephemeral=True,
                )
                return

            await game_cog.service.start_game(interaction)  # type: ignore[attr-defined]
            return
