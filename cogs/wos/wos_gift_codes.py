...
def _normalise_blob(raw: Any) -> dict[str, Any]:
    ...
    codes = raw.get("codes")
    validation_candidates = raw.get("validation_candidates")
...
def _looks_like_bare_code(candidate: str) -> bool:
    ...
    # WoS also releases all-letter codes. Accept either:
    #   - fully uppercase codes such as OFFICIALSTORE; or
    #   - deliberately mixed-case codes with an uppercase character after the
    #     first position, such as gogowOS or GoGoWOS.
    #
    # A normal sentence-case word such as Morning or Discord is ignored because
    # its only uppercase character is the first one. Fully lowercase chat is
    # ignored as well.
    if candidate.isalpha():
        upper = candidate.upper()
        has_lower = any(char.islower() for char in candidate)
        has_internal_upper = any(char.isupper() for char in candidate[1:])
        code_style = candidate == upper or (has_lower and has_internal_upper)
        return (
            code_style
...
class WOSGiftCodesCog(commands.Cog):
    ...
    def __init__(self, bot: commands.Bot):
        ...
        self._validation_queue: asyncio.Queue[tuple[int, str]] = asyncio.Queue()
        self._validation_queued_keys: set[tuple[int, str]] = set()
        self._validation_cooldowns: dict[tuple[int, str], float] = {}
        self._validation_user_cooldowns: dict[tuple[int, int], float] = {}
        self._validation_retry_tasks: set[asyncio.Task[None]] = set()
        self._progress_locks: dict[tuple[int, str], asyncio.Lock] = {}
        self._progress_cleanup_done: set[tuple[int, str]] = set()
        self._worker_task: asyncio.Task[None] | None = None
        self._validation_worker_task: asyncio.Task[None] | None = None
    ...
    async def cog_load(self) -> None:
        ...
        self._validation_worker_task = asyncio.create_task(
            self._validation_worker(),
            name="hotbot-wos-gift-validation-worker",
        )
        await self._recover_validation_candidates()
    ...
    def cog_unload(self) -> None:
        ...
        if self._validation_worker_task is not None:
            self._validation_worker_task.cancel()
        for task in tuple(self._validation_retry_tasks):
            task.cancel()
    ...
    def _enqueue_validation(self, guild_id: int, code: str) -> None:
        key = (int(guild_id), str(code).casefold())
        if key in self._validation_queued_keys:
            return
        self._validation_queued_keys.add(key)
        self._validation_queue.put_nowait((int(guild_id), str(code)))

    def _schedule_validation_retry(self, guild_id: int, code: str, delay: float) -> None:
        task = asyncio.create_task(
            self._delayed_validation_enqueue(guild_id, code, delay),
            name=f"hotbot-wos-gift-validation-retry-{guild_id}-{code}",
        )
        self._validation_retry_tasks.add(task)
        task.add_done_callback(self._validation_retry_tasks.discard)

    async def _delayed_validation_enqueue(
        self,
        guild_id: int,
        code: str,
        delay: float,
        ...
        self._enqueue_validation(guild_id, code)

    async def _recover_validation_candidates(self) -> None:
        now = time.time()
        recovered = 0

        for guild_id in public_guild_ids(self.bot, include_admin=True):
            queue_now: list[str] = []
            schedule_later: list[tuple[str, float]] = []

            async with self._lock_for(guild_id):
                blob = self._load_blob(guild_id)
                candidates = blob["validation_candidates"]
                changed = False

                for stored_code in list(candidates.keys()):
                    candidate = candidates.get(stored_code)
                    if not isinstance(candidate, dict):
                        del candidates[stored_code]
                        changed = True
                        continue

                    # If another path already promoted it, discard the stale
                    # hidden candidate instead of validating it again.
                    if _find_stored_code_key(blob["codes"], str(stored_code)) is not None:
                        del candidates[stored_code]
                        changed = True
                        continue
                    ...
                    if status in {"queued", "validating"}:
                        ...
                        queue_now.append(str(stored_code))
                        changed = True
                    elif status == "retry_wait":
                        try:
                            next_retry_at = float(candidate.get("next_retry_at") or 0)
                        except (TypeError, ValueError):
                            next_retry_at = 0
                        ...
                        if next_retry_at <= now:
                            ...
                            schedule_later.append((str(stored_code), next_retry_at - now))

                if changed:
                    self._save_blob(guild_id, blob)

            for stored_code in queue_now:
                self._enqueue_validation(guild_id, stored_code)
            for stored_code, delay in schedule_later:
                self._schedule_validation_retry(guild_id, stored_code, delay)
            recovered += len(queue_now) + len(schedule_later)

        info(f"WoS gift-code validation recovery found {recovered} candidate(s)")

    async def _prepare_auto_validation(
        self,
        *,
        guild_id: int,
        channel_id: int,
        ...
        if not CODE_RE.fullmatch(clean_code):
            return "invalid_format", clean_code

        # Stored, already-proven codes keep the normal repost/retry behaviour
        # and must not be blocked by the anti-spam validation cooldown.
        async with self._lock_for(guild_id):
            blob = self._load_blob(guild_id)
            stored_code_key = _find_stored_code_key(blob["codes"], clean_code)
            if stored_code_key is not None:
                return "stored", stored_code_key

        now_monotonic = time.monotonic()
        user_cooldown_key = (int(guild_id), int(submitter_id))
        last_user_candidate = self._validation_user_cooldowns.get(user_cooldown_key, 0.0)
        if now_monotonic - last_user_candidate < AUTO_VALIDATION_USER_COOLDOWN_SECONDS:
            return "cooldown", clean_code

        cooldown_key = (int(guild_id), clean_code.casefold())
        last_seen = self._validation_cooldowns.get(cooldown_key, 0.0)
        if now_monotonic - last_seen < AUTO_VALIDATION_COOLDOWN_SECONDS:
            return "cooldown", clean_code

        self._validation_user_cooldowns[user_cooldown_key] = now_monotonic
        self._validation_cooldowns[cooldown_key] = now_monotonic

        async with self._lock_for(guild_id):
            blob = self._load_blob(guild_id)
            # Recheck after acquiring the lock because a validation worker may
            # have promoted the candidate between the first check and now.
            stored_code_key = _find_stored_code_key(blob["codes"], clean_code)
            if stored_code_key is not None:
                return "stored", stored_code_key

            candidates = blob["validation_candidates"]
            candidate_key = _find_stored_code_key(candidates, clean_code)
            if candidate_key is not None:
                clean_code = candidate_key
            ...
            if not isinstance(candidate, dict):
                ...
                candidate["channel_id"] = int(channel_id)
                candidate["source_message_id"] = int(source_message_id or 0)
                candidate["submitted_by"] = int(submitter_id)
                candidate["status"] = "queued"
                candidate["validation_attempts"] = 0
                candidate.pop("next_retry_at", None)

            candidates[clean_code] = candidate
            self._save_blob(guild_id, blob)

        self._enqueue_validation(guild_id, clean_code)
        return "validating", clean_code
    ...
    async def _validation_snapshot(
        ...
        async with self._lock_for(guild_id):
            blob = self._load_blob(guild_id)
            candidate_key = _find_stored_code_key(blob["validation_candidates"], code)
            if candidate_key is None:
                return None
            candidate = blob["validation_candidates"].get(candidate_key)
            if not isinstance(candidate, dict):
                return None

            candidate["status"] = "validating"
            candidate["last_validation_started_at"] = _utc_now()
            candidate["validation_attempts"] = int(candidate.get("validation_attempts") or 0) + 1
            self._save_blob(guild_id, blob)
            ...
            for fid, account in blob["accounts"].items():
                if not isinstance(account, dict) or not account.get("enabled", True):
                    continue
                accounts[str(fid)] = dict(account)

            return str(candidate_key), dict(candidate), accounts

    async def _set_validation_waiting_for_account(
        self,
        guild_id: int,
        code: str,
        *,
        ...
        async with self._lock_for(guild_id):
            ...
            if not isinstance(candidate, dict):
                return
            candidate["status"] = "waiting_for_account"
            candidate["updated_at"] = _utc_now()
            if isinstance(last_result, dict):
                candidate["last_validation_result"] = dict(last_result)
            self._save_blob(guild_id, blob)

    async def _defer_validation_candidate(
        self,
        guild_id: int,
        code: str,
        result: dict[str, Any],
        ...
        async with self._lock_for(guild_id):
            ...
            attempts = int(candidate.get("validation_attempts") or 0)
            candidate["last_validation_result"] = dict(result)
            candidate["updated_at"] = _utc_now()

            retry_index = attempts - 1
            if 0 <= retry_index < len(AUTO_VALIDATION_RETRY_DELAYS_SECONDS):
                delay = float(AUTO_VALIDATION_RETRY_DELAYS_SECONDS[retry_index])
                candidate["status"] = "retry_wait"
                candidate["next_retry_at"] = time.time() + delay
            else:
                # Reposting the candidate or registering/updating an account can
                # wake it again. Pausing prevents endless API traffic.
        ...
        if delay is not None:
            self._schedule_validation_retry(guild_id, code, delay)

    async def _remove_validation_candidate(self, guild_id: int, code: str) -> None:
        async with self._lock_for(guild_id):
            blob = self._load_blob(guild_id)
            candidate_key = _find_stored_code_key(blob["validation_candidates"], code)
            if candidate_key is None:
                return
            del blob["validation_candidates"][candidate_key]
            self._save_blob(guild_id, blob)

    async def _promote_validated_candidate(
        self,
        *,
        guild_id: int,
        code: str,
        ...
        async with self._lock_for(guild_id):
            ...
            candidate_key = _find_stored_code_key(blob["validation_candidates"], code)
            candidate = (
                blob["validation_candidates"].get(candidate_key)
                if candidate_key is not None
                else None
            )
            if not isinstance(candidate, dict):
            ...
            stored_code = str(candidate.get("code") or candidate_key or code)
            channel_id = int(candidate.get("channel_id") or 0)
            source_message_id = int(candidate.get("source_message_id") or 0)
            existing_key = _find_stored_code_key(blob["codes"], stored_code)

            if existing_key is None:
                entry = {
                    "code": stored_code,
                    "submitted_at": str(candidate.get("created_at") or _utc_now()),
                    "submitted_by": int(candidate.get("submitted_by") or 0),
                    "channel_id": channel_id,
                    "source_message_id": source_message_id,
                    "status_message_id": 0,
                    "status": "queued",
                    "results": {str(fid): dict(validation_result)},
                    "validated_at": _utc_now(),
                    "validated_by_fid": str(fid),
                    "validated_result": str(validation_result.get("status") or ""),
                }
                blob["codes"][stored_code] = entry
            else:
                stored_code = existing_key
                entry = blob["codes"].get(existing_key)
                if isinstance(entry, dict):
                    results = entry.setdefault("results", {})
                    if not isinstance(results, dict):
                        results = {}
                        entry["results"] = results
                    results.setdefault(str(fid), dict(validation_result))
                    entry["status"] = "queued"
                    entry["channel_id"] = channel_id or int(entry.get("channel_id") or 0)

            if candidate_key is not None:
                del blob["validation_candidates"][candidate_key]
            self._save_blob(guild_id, blob)

        # The validation request already produced the first account's real
        # result. Save it and process only the remaining accounts.
        self._enqueue(guild_id, stored_code)
        await self._publish_progress(guild_id, stored_code)
        await self._add_reaction_to_source(
            guild_id=guild_id,
            channel_id=channel_id,
    ...
    async def _validation_worker(self) -> None:
        while True:
            guild_id, code = await self._validation_queue.get()
            self._validation_queued_keys.discard((int(guild_id), str(code).casefold()))
            try:
                await self._validate_candidate(guild_id, code)
            except asyncio.CancelledError:
                raise
            except Exception as exc:
                err(
                    f"WoS gift-code validation failed for guild {guild_id}, "
                    f"candidate {code}: {exc!r}"
                )
                await self._defer_validation_candidate(
                    guild_id,
                    code,
                    {
                        "status": "api_error",
                        "err_code": None,
                        "message": f"Validation worker error: {exc}",
    ...
    async def _validate_candidate(self, guild_id: int, code: str) -> None:
        snapshot = await self._validation_snapshot(guild_id, code)
        if snapshot is None:
            return

        stored_code, candidate, accounts = snapshot
        if not accounts:
            await self._set_validation_waiting_for_account(guild_id, stored_code)
            return

        ordered_accounts = sorted(
            accounts.items(),
            key=lambda item: (
                str(item[1].get("registered_at") or ""),
                str(item[0]),
            ),
        )
        last_inconclusive: dict[str, Any] | None = None

        for fid, account in ordered_accounts:
            try:
            ...
            result = await self._redeem_one(fid=fid, kid=kid, code=stored_code)
            result_status = str(result.get("status") or "failed")

            if result_status in VALIDATION_PROVES_CODE_STATUSES:
                await self._promote_validated_candidate(
                    guild_id=guild_id,
                    code=stored_code,
                    fid=fid,
            ...
            if result_status in VALIDATION_REJECTS_CODE_STATUSES:
                # Fake, expired and globally exhausted candidates stay silent:
                # no progress panel and no reaction.
                await self._remove_validation_candidate(guild_id, stored_code)
                info(
                    f"Ignored WoS gift-code candidate {stored_code!r} in guild "
                    f"{guild_id}: {result_status}"
                )
                return

            if result_status == "kid_mismatch":
                last_inconclusive = result
                continue

            # Network failures, throttling, an API change and unknown responses
            # prove nothing about the candidate. Retry later without posting.
            last_inconclusive = result
            break
        ...
        if last_inconclusive is None:
            last_inconclusive = {
                "status": "failed",
                "err_code": None,
                "message": "No account could validate this candidate.",
                "attempted_at": _utc_now(),
                "attempts": 0,
            }

        if str(last_inconclusive.get("status") or "") == "kid_mismatch":
            await self._set_validation_waiting_for_account(
                guild_id,
                stored_code,
                last_result=last_inconclusive,
            )
        else:
            await self._defer_validation_candidate(
                guild_id,
    ...
    async def _wake_validation_candidates_after_registration(
        self,
        *,
        guild_id: int,
        channel_id: int,
        ...
        async with self._lock_for(guild_id):
            ...
            for stored_code, candidate in candidates.items():
                if not isinstance(candidate, dict):
                    continue
                status = str(candidate.get("status") or "queued")
                if status not in {"waiting_for_account", "paused", "retry_wait"}:
                    continue
                candidate["status"] = "queued"
                candidate["channel_id"] = int(channel_id)
                candidate["submitted_by"] = int(submitter_id)
    ...
    async def on_message(self, message: discord.Message) -> None:
        ...
        for code in codes:
            validation_state, resolved_code = await self._prepare_auto_validation(
            ...
            if validation_state != "stored":
                # New candidates stay completely silent until WoS proves they
                # are genuine. A valid restricted/already-used response still
                # promotes the code and the validation worker adds ✅ later.
                continue
    ...
    async def register(
        ...
        await self._wake_validation_candidates_after_registration(
            guild_id=guild_id,
            channel_id=int(interaction.channel_id or 0),
            submitter_id=user_id,
        )
